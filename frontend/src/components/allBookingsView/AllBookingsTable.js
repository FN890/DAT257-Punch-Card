...
export default function AllBookingsTable() {
    ...
    function setAllBookings(){
        bookingService.getArchivedBookings(cookies.JWT).then(data => {
            setArchivedBooking(data.data)
        }).catch(() => history.push("/loggain"));
        bookingService.getNotArchivedBookings(cookies.JWT).then(data => {
            let active = []
            let old = []
            for (let i = 0; i < data.data.length; i++) {
                if(moment().isAfter(data.data[i].endTime)){
                    old.push(data.data[i])
                }
                else {
                    active.push(data.data[i])
                }
            }
            setBookings(active)
            setFinishedBooking(old)
        }).catch(() => history.push("/loggain"));
    }
    ...
    const onClickPaid = (event) => {
        bookingService.updateBooking(event.id, event.description, event.responsible, !event.paid, event.price, event.customer,event.archived, cookies.JWT).then(() => {
                setAllBookings()
        })
    }
